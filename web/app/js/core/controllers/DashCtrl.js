...
app.controller('DashCtrl', ['$rootScope','$scope','$state','$stateParams','authUser', function($rootScope,$scope,$state,$stateParams,authUser) {
  ...
  $(function () {
         ...
         $('#sub').submit(function(e){
           e.preventDefault();
           console.log("3");
           $scope.socket.emit('chat message', $('#m').val(),-1);
           console.log("4");
           $('#m').val('');
           return false;
         });
         $scope.socket.on('chat message', function(msg,id){
           console.log(id);
           console.log("5");
           if (id ==  1){
             console.log("6");
...
var val = $('#searchValue').val().toLowerCase() || $('select[name=selector]').val();
 ...
 var val = $scope.globalSearch.toLowerCase();
  ...
  //     console.log("1");
  //     $scope.check += 1
  //     $scope.socket.emit('chat message', $scope.myMessage,$scope.check );
  //     console.log("2");
  //   }
  // }

  $('#mbut').unbind('click').click(function() {
    alert("bob");
    var mssg = $("#m").val()
    $scope.socket.emit('chat message', mssg,$scope.check );
});
