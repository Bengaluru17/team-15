...
workbook = excelSheet( excelPath )
fnames = [a.encode('ascii','ignore') for a in workbook.getDataColFromExcel( 'A1' )]
lnames = [a.encode('ascii','ignore') for a in workbook.getDataColFromExcel( 'B1' )]
college = [a.encode('ascii','ignore') for a in workbook.getDataColFromExcel( 'C1' )]
marks = workbook.getDataColFromExcel( 'D1' )

for i in range(len(fnames)):
	in_data = db.data.insert_one({"name":fnames[i],"surname": lnames[i], "school": college[i], "marks": marks[i]})
